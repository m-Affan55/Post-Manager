...
export async function AddComment(comment) {
    try {
        const response = await apiFetch(`${Base_url}/${comment.post_id}`, {
            method: "POST",
...
export async function UpdateComment(comment) {
    try {
        const response = await apiFetch(`${Base_url}/${comment.id}`, {
            method: "PUT",
...
export async function DeleteComment(comment_id) {
    try {
        const response = await apiFetch(`${Base_url}/${comment_id}`, {
            method: "DELETE",
        });
        if (!response.ok) {
            const err = await response.json().catch(() => ({}));
